...
export const enum SCgObjectState {
    Normal = 0,
    MergedWithMemory = 1,
    NewInMemory = 2,
    FromMemory = 3,
...
export const enum SCgObjectLevel {
    First = 0,
    Second = 1,
    Third = 2,
    Fourth = 3,
...
export class ModelObject {
    ...
    setScAddr(addr: number, isCopy = false) {
        if (this.scene) {
            if (isCopy) {
                this.sc_addr = addr;
                const targetObject = this.scene.objects.get(this.sc_addr);
                if (targetObject) {
                    targetObject.copies[this.id] = this;
                } else {
                    console.error(`Target object not found for sc_addr: ${this.sc_addr}`);
                }
            } else {
                // remove old sc-addr from map
                if (this.sc_addr && this.scene.objects.get(this.sc_addr)) {
                    this.scene.objects.delete(this.sc_addr);
                }
                this.sc_addr = addr;

                if (this.sc_addr) this.scene.objects.set(this.sc_addr, this);
            }
...
export class ModelContour extends ModelObject {
    ...
    isNodeInPolygon(node: ModelNode) {
        return Algorithms.isPointInPolygon(node.position, this.points);
    };
